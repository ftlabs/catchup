...
function constructSAPIQuery( params ) {
	...
	const defaults = {
	  ...
	  constraints   : [],
		constraintsOr : [],
		ontologies    : ["people", "organisations", "topics"],
	};
	...
	if (queryString == ''){
		...
		if(combined.constraints.length > 0 ) {
			// NB: not promises...
			ands = combined.constraints.map(c => { return rephraseEntityForQueryString(c); });
		}
		if (combined.constraintsOr.length > 0) {
			const ors = combined.constraintsOr.map(c => {return rephraseEntityForQueryString(c); });
	...
	if (combined.ontologies && combined.ontologies.length > 0) {
		full.resultContext.facets = {
			names       : combined.ontologies,
			maxElements : -1,
		};
	}
